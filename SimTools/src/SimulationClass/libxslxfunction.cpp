...
void Simulation::writeDataToXLSX(std::string filename)
{
	...
	lxw_workbook* wb{ fileIO::xlsx::writeDblArray(fileIO::xlsx::openNewWorkbook(filename), "EscapedElec", writedata,{ 1,0 }) };
	writedata.clear();
	...
	fileIO::xlsx::writeDblArray(wb, "EscapedElec", writedata, { 1,4 });
	writedata.clear();
	...
	fileIO::xlsx::writeDblArray(wb, "EscapedElec", writedata, { 1,9 });
	writedata.clear();
	...
	fileIO::xlsx::writeDblArray(wb, "EscapedElec", writedata, { 1,14 });
	fileIO::xlsx::closeAndSave(wb);
}