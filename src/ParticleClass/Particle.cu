...
void Particle::clearGPUMemory()
{
	...
	{
		CUDA_API_ERRCHK(cudaMemset(origData1D_d, 0.0, sizeof(double) * (numberOfPositionDims_m + numberOfVelocityDims_m) * numberOfParticles_m));
		CUDA_API_ERRCHK(cudaMemset(currData1D_d, 0.0, sizeof(double) * (numberOfPositionDims_m + numberOfVelocityDims_m) * numberOfParticles_m));
	}
}