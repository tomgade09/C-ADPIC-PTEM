...
__global__ void satelliteDetector(double** data_d, double** capture_d, double simtime, double dt, double altitude, bool upward)
{
		...
		)
	{
		...
		double s_ratio{ (altitude - s0_d[thdInd]) / (s_d[thdInd] - s0_d[thdInd]) };
		double interpol_v_d{ v0_d[thdInd] + (v_d[thdInd] - v0_d[thdInd]) * s_ratio }; //interpolates v back to where the particle was detected
		double interpol_t_d{ simtime + dt * s_ratio };
		//
		//
		//
