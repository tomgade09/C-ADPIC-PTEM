...
def initializeSimCPP():
    simDLL.newSimInitialize.argtypes = None
    simDLL.newSimInitialize.restype = ctypes.c_void_p
    simulationptr = ctypes.c_void_p
    simulationptr = simDLL.newSimInitialize()
...
def terminateSimulationCPP(simulationptr):
...
def returnResultsCPP(simulationptr):
...
def getEatZCPP(LUT, z, simtime):
    simDLL.getEatZ.argtypes = (ctypes.c_void_p, ctypes.c_double, ctypes.c_double)
    simDLL.getEatZ.restype = ctypes.c_double

    return simDLL.getEatZ(LUT, z, simtime)

def getBatZCPP(z, simtime):
    simDLL.getBatZ.argtypes = (ctypes.c_double, ctypes.c_double)
    simDLL.getBatZ.restype = ctypes.c_double

    return simDLL.getBatZ(z, simtime)
