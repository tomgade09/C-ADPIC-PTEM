...
constexpr double CONSTEFIELD{ TOTPOTDROP / (MAX_Z_FROM_RE - MIN_Z_FROM_RE) }; //E Field centered at 2 Re, spread out 2000 km, V / m
...
const double	 V_SIGMA{ sqrt(INITIAL_T_EV * 1.60218e-19 / MASS_ELECTRON) }; //in units of Re / s  (if normalized), otherwise m / s
constexpr double V_SIGMA_SQ{ INITIAL_T_EV * 1.60218e-19 / (MASS_ELECTRON) }; //need this for cuda code
