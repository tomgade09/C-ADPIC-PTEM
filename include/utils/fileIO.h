...
namespace utils
{
	namespace fileIO
	{
		...
		DLLEXP void writeDblBin(const std::vector<double>& dataarray, std::string filename, size_t numelements, bool overwrite = true);
		DLLEXP void write2DCSV(const std::vector<std::vector<double>>& dataarray, std::string filename, size_t numofentries, size_t numofcols, const char delim = ',', bool overwrite = true, int precision = 20);
